...
extension UIView {
    ...
    /// Adds a constraint (leading) that defines one item’s attribute as equal to another item’s attribute after the item anchor plus a constant offset.
    /// - Parameters:
    ///   - anchor: A layout anchor.
    ///   - constant: The constant offset for the constraint.
    @discardableResult
    func leadingAnchor(after anchor: NSLayoutXAxisAnchor, constant: CGFloat = 0) -> Self {
        translatesAutoresizingMaskIntoConstraints = false
        leadingAnchor.constraint(equalToSystemSpacingAfter: anchor, multiplier: constant).isActive = true
        return self
    }
